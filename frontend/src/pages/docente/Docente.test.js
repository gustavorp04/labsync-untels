...
import DocenteReservar from './Reservar';
import DocenteMisReservas from './MisReservas';
...
describe('PBI-20 — Flujo crítico de reservas (Docente)', () => {
  ...
  test('(1) docente crea una reserva válida recorriendo los 5 pasos del wizard', async () => {
    ...
    await waitFor(() =>
      expect(mockShowToast).toHaveBeenCalledWith('ok', expect.stringMatching(/Reserva creada exitosamente/i))
    );
  });
