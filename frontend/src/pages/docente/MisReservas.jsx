...
function DocenteMisReservas() {
  ...
  return (
    <div className="doc-section">
                        ...
                        <div key={i} style={{ position:'relative',marginBottom:i < reservaDetalle.historial_cambios.length - 1 ? 14 : 0,paddingLeft:20 }}>
                          ...
                          <div style={{ background:'var(--bg-input)',border:`1px solid var(--border-color)`,borderLeft:`3px solid ${dotColor}`,borderRadius:8,padding:'8px 12px' }}>
                            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:6 }}>
                              <span style={{ fontSize:13 }}><span style={{ opacity:0.6 }}>{log.estado_anterior}</span><span style={{ margin:'0 6px',opacity:0.4 }}>→</span><strong style={{ color:dotColor }}>{log.estado_nuevo}</strong></span>
      ...
      <div className="doc-section-header"><h2>Mis Reservas</h2></div>
      {misReservas.length === 0 ? (
        <div className="doc-empty-state">
          <div style={{ fontSize:48,marginBottom:12 }}>📋</div>
          <p>Aún no tienes reservas registradas.</p>
          <button className="doc-btn-primary" onClick={() => navigate('/docente/reservar')}>Crear mi primera reserva</button>
        </div>
      ) : (
          ...
          {misReservas.map(r => (
            <div key={r.id_reserva} className="doc-reserva-card">
              ...
              <div className="doc-reserva-right">
                ...
                <button onClick={() => setReservaDetalle(r)} style={{ padding:'4px 12px',fontSize:12,background:'var(--bg-input)',border:'1px solid var(--border-color)',borderRadius:4,cursor:'pointer',color:'var(--text-main)' }}>
                  Ver detalle
                </button>
                {r.estado === 'Programada' && (
...
export default DocenteMisReservas;
